...
public class DiamondSquareMapGenerator : MonoBehaviour {
    ...
    private Mesh createCubeMesh() {
        ...
        for (int i=0; i < arrayLength-1; i++) {
            for (int j = 0; j < arrayLength-1; j++) {
                ...
                vertList.Add(points[i, j + 1]);
                vertList.Add(points[i + 1, j]);

                //Triangle 1 Colours.
                vertColourList.Add(getColourBasedOnHeight(points[i, j]));
                vertColourList.Add(getColourBasedOnHeight(points[i, j+1]));
                vertColourList.Add(getColourBasedOnHeight(points[i+1, j]));

                //Triangle 2 Positions.
                vertList.Add(points[i, j + 1]);
                vertList.Add(points[i + 1, j + 1]);
                vertList.Add(points[i + 1, j]);

                //Triangle 2 Colours.
                vertColourList.Add(getColourBasedOnHeight(points[i, j+1]));
                vertColourList.Add(getColourBasedOnHeight(points[i+1, j + 1]));
                vertColourList.Add(getColourBasedOnHeight(points[i + 1, j]));

                /*                //If we have super high resolution, we will eventually exceed the maximum number of vertices for a mesh.
                                //Thus, we might have to make two meshes for this gameobject

                                if (vertList.Count >= 64980) {
                                    //Mesh has gotten full. We hsould assign this mesh array
                                    m.vertices = vertList.ToArray();
                                    triangles = new int[m.vertices.Length];
                                    for (int k = 0; k < m.vertices.Length; k++) {
                                        triangles[k] = k;
                                    }
                                    m.triangles = triangles;
                                    cubeMesh.mesh = m;      //ADD THE MESH TO THE MESH FILTER

                                    //Add new mesh to this gameobject, and make a new mesh filter and mesh
                                    cubeMesh = this.gameObject.AddComponent<MeshFilter>();  //CREATE NEW MESH FILTER
                                    m = new Mesh();                                         //CREATE NEW MESH OBJECT
                                    vertList = new List<Vector3>();                         //NEW LIST FOR VERTEX STORAGE
                                }
                */
            }
        }
